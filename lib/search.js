 ...
 * @function
 * @param {object} options - config object
 * @param {string|string[]} options.attributeName - The name of the attribute to filter on.
 * @param {boolean|boolean[]} [options.exists] - The value indicating whether the attribute should exist or not.
 * @param {object} options.expressionAttributeNames - The expression attribute names object to add the attribute name to.
 * @param {string[]} options.filterConditions - The filter conditions array to add the filter condition to.
 * @param {'AND'|'OR'} [options.operator] - The operator to use for the filter condition.
 * @return {undefined}
 */
export const addFilterConditionExists = ({
  ...
  attributeName = _.castArray(attributeName);
  exists = _.castArray(exists);

  if (attributeName.length !== exists.length)
    throw createError.BadRequest(
      'attributeName and exists must be the same length'
    );

  const conditions = [];

  for (let i = 0; i < attributeName.length; i++) {
    if (_.isUndefined(exists[i])) continue;
    expressionAttributeNames[`#${attributeName[i]}`] = attributeName[i];
    conditions.push(
      `attribute${exists[i] ? '' : '_not'}_exists(#${attributeName[i]})`
    );
  }

  if (conditions.length)
    filterConditions.push(`(${conditions.join(` ${operator} `)})`);
};
...
export const addFilterConditionRange = ({
  ...
  if (!_.isUndefined(attributeValueFrom))
    expressionAttributeValues[`:${attributeName}From`] = attributeValueFrom;

  if (!_.isUndefined(attributeValueTo))
    expressionAttributeValues[`:${attributeName}To`] = attributeValueTo;

  if (!_.isUndefined(attributeValueFrom) && _.isUndefined(attributeValueTo))
    filterConditions.push(`#${attributeName} >= :${attributeName}From`);
  else if (
    _.isUndefined(attributeValueFrom) &&
    !_.isUndefined(attributeValueTo)
  )
...
export const getSortKeyConditionRange = ({
  ...
  if (!_.isUndefined(sortKeyValueFrom))
    expressionAttributeValues[`:${sortKeyName}From`] = sortKeyValueFrom;

  if (!_.isUndefined(sortKeyValueTo))
    expressionAttributeValues[`:${sortKeyName}To`] = sortKeyValueTo;

  return !_.isUndefined(sortKeyValueFrom) && _.isUndefined(sortKeyValueTo)
    ? `#${sortKeyName} >= :${sortKeyName}From`
    : _.isUndefined(sortKeyValueFrom) && !_.isUndefined(sortKeyValueTo)
...
export const parseWholeNumberQueryParam = (param, token) => {
  ...
  if (_.lowerCase(param) === 'all') return Infinity;
