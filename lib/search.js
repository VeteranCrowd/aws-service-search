 ...
 * @param {boolean} [options.negate=false] - Whether to negate the filter condition.
...
export const addFilterCondition = ({
  ...
  if (operator === 'in' && !_.isArray(attributeValue))
  ...
  if (operator !== 'in' && _.isArray(attributeValue))
    throw createError.BadRequest(
      `'${attributeName}' attributeValue value must be a string`
    );
  ...
  if (operator === 'in')
    for (const value of attributeValue)
      expressionAttributeValues[`:${attributeName}_${value}`] = value;
 ...
 * @param {boolean} [options.scanIndexForward=true] - Whether to scan the index forward.
