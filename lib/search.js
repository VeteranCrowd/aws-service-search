...
export const rangeKeyConditionBeginsWith = ({
  ...
  const keyValue = encode(item)[keyToken];

  if (!keyValue) throw new error.BadRequest('invalid key value');
...
export const rangeKeyConditionRange = ({
  ...
  const keyValueFrom = encode(itemFrom)[keyToken];
  const keyValueTo = encode(itemTo)[keyToken];

  if (!keyValueFrom && !keyValueTo)
    throw new error.BadRequest('invalid key range');
